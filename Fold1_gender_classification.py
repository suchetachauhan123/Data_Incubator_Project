...
n_classes = 1
VALIDATION_SIZE = 24  # Size of the validation set.
...
def main(argv=None):  # pylint: disable=unused-argument
  ...
  path = os.getcwd()
  directory = 'gender_network'
  ffrom = 0
  h = 100
  lr = 0.001
  reg = 0.01
  std = 0.1
  f = 8		# number of feature maps
  NUM_EPOCHS = 50
  filter1 = 5	# filter size
  to = ffrom + 2

  os.makedirs(directory)
  new_path= os.path.join(path, directory)

  inp = np.load('train_gender.npz')['input']
  tar = np.load('train_gender.npz')['labels']
  ...
  inp=inp[VALIDATION_SIZE:]
  tar=tar[VALIDATION_SIZE:]
  ...
  conv1_weights = tf.Variable(
      tf.truncated_normal([filter1, filter1, filter1, NUM_CHANNELS, f],  # 5x5 filter, depth 8.
                          stddev=std,
                          seed=SEED))
  ...
  print(logits.get_shape(), train_labels_node.get_shape())
    ...
    for step in xrange((int(num_epochs * train_size) // BATCH_SIZE)):
      ...
      if step == (i * no_of_batches) - 1:
	      ...
	      batch_labels = batch_labels[idx]
              ...
              print('Training error: %.3f, validation error: %.3f, learning rate: %.6f' % (l, tr_err, val_err, lr))
              print('---------------------------------------------------------------------------------------------------------------')
